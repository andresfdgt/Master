...
@section('contenido')
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="page-title-box">
                    <h4 class="page-title">Editar tipo de porte</h4>
                </div>
                <!--end page-title-box-->
            </div>
            <!--end col-->
        ...
        <div class="card">
            <div class="card-body">
                <form class="needs-validation" enctype="multipart/form-data" novalidate id="editarTipoPorte" method="post">
                    @csrf
                    @method('put')
                    ...
                    <div class="form-row">
                        <div class="col-md-3 form-group">
                            ...
                            <input type="hidden" name="id" value="{{ $tipoPorte->id }}">
                            <input type="text" class="form-control" name="descripcion"
                                value="{{ $tipoPorte->descripcion }}" required>
                        ...
                        <div class="checkbox my-2">
                            <div class="custom-control custom-checkbox">
                                <input type="checkbox" class="custom-control-input" id="favorito" name="favorito"
                                    value="1" {{ $tipoPorte->favorito == 1 ? 'checked' : '' }}>
                        ...
                        <div class="col-md-12">
                            <div class="card">
                                ...
                                <div class="card-body">
                                    ...
                                    <textarea id="observaciones" cols="30" rows="10">{{ $tipoPorte->observaciones }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-row">
                        <div class="col-md-12 text-center">
                            <button class="btn btn-primary btn-lg" id="actualizarTipoPorte" type="submit">Editar
                                tipo de porte</button>
                        </div>
                    </div>
                </form>
            </div>
...
@section('add-scripts')
    ...
    <script>
        ...
        $("#editarTipoPorte").submit(function(e) {
            e.preventDefault();
            $('#editarTipoPorte').addClass('was-validated');
            if ($('#editarTipoPorte')[0].checkValidity() === false) {
                ...
                $.ajax({
                    type: "post",
                    url: "/tipo_portes",
                    data: new FormData($('#editarTipoPorte')[0]),
                    contentType: false,
                    cache: false,
                    processData: false,
                    beforeSend: function() {
                        $("#actualizarTipoPorte").html(
                            "<i class='fa fa-spinner fa-pulse'></i><span class='sr-only'> Actualizando...</span>Actualizando..."
                        );
                        $("#actualizarTipoPorte").attr("disabled", true);
                    },
                    dataType: "json",
                    success: function(data) {
                        $("#actualizarTipoPorte").html("Actualizado");
                        $("#actualizarTipoPorte").removeAttr("disabled");
                        if (data.status == "success") {
                            $('#editarTipoPorte').removeClass('was-validated');
